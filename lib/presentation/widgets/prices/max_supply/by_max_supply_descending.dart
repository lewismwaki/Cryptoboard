...
import 'package:supercharged/supercharged.dart';import 'package:cryptoboard/presentation/shared/toast_notification.dart';
...
class ByMaxSupplyDescending extends StatelessWidget {
  final int index;
  ByMaxSupplyDescending({this.index});
    ...
    return BlocBuilder<AssetsCubit, AssetsState>(
      ...
      builder: (context, state) {
        if (state is AssetsSortedByMaxSupplyDescending) {
          ...
          final sortedByMaxSupplyDescending =
              assetClass.sortedByNum((element) => element.maxSupply == null ? 0 : double.parse(element.maxSupply)).reversed.toList();
          final reversedAsset = sortedByMaxSupplyDescending[index];
